...
from app.models import AiringStatus, Anime, Genre, MediaFormat, MediaSource
...
class AniListService:
    ...
    def save_anime_to_db(self, anime_data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            existing = (
                db.query(Anime).filter(Anime.external_id == anime_data["id"]).first()
            )
            if existing:
                print(
                    f"ℹ️ Anime {anime_data['title']['romaji']} já existe. ID: {existing.id}"
                )
                return existing.id

            title = (
                anime_data.get("title", {}).get("english")
                or anime_data.get("title", {}).get("romaji")
                or anime_data.get("title", {}).get("native")
                or "Unknown"
            )
    ...
    def search_anime(self, query: str, per_page: int = 10) -> List[Dict[str, Any]]:
        ...
        response = self.client.post(
            ...
            json={
                "query": graphql_query,
                "variables": {"query": query, "perPage": per_page},
            },
        )
        response.raise_for_status()
    ...
    def search_anime_by_id(self, anime_id: int) -> Optional[Dict[str, Any]]:
        ...
        response = self.client.post(
            self.BASE_URL, json={"query": graphql_query, "variables": {"id": anime_id}}
        )
        response.raise_for_status()
