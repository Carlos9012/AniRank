...
def get_user_anime_matrix(db: Session) -> pd.DataFrame:
    records = db.query(UserAnimeStatus).filter(UserAnimeStatus.score.isnot(None)).all()
    ...
    data = []
    for r in records:
        data.append({"user_id": r.user_id, "anime_id": r.anime_id, "score": r.score})
...
def get_user_rated_animes(db: Session, user_id: int) -> dict:
    records = (
        ...
        .filter(UserAnimeStatus.user_id == user_id, UserAnimeStatus.score.isnot(None))
        .all()
    )
