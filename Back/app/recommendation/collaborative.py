...
def recommend_by_collaborative(
    user_id: int, matrix: pd.DataFrame, top_n: int = 5
) -> List[Dict[str, Any]]:
    if matrix.empty:
        return []
    ...
    anime_similarity_df = pd.DataFrame(
        anime_similarity, index=matrix.columns, columns=matrix.columns
    )
    ...
    return [
        {"anime_id": int(anime_id), "predicted_score": round(score, 2)}
...
def get_anime_similarity_matrix(matrix: pd.DataFrame) -> pd.DataFrame:
    anime_similarity = cosine_similarity(matrix.T)
    return pd.DataFrame(anime_similarity, index=matrix.columns, columns=matrix.columns)
